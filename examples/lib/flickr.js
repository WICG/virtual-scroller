...
export async function searchFlickr(query) {
    const ret = await jsonp(
        `https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=${apiKey}&text=${query}&sort=interestingness-desc&extras=o_dims%2C+url_o&format=json`,
        {
            param: 'jsoncallback'
        }
    ).promise;
