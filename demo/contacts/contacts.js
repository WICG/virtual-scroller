...
const scrollMethod =
    document.createElement('div').scrollIntoViewIfNeeded ? 
...
export class Sample {
    constructor() {
        ...
        this.resetValue = {
            image: 'http://via.placeholder.com/73x73&text=+'
        };

        // TODO: Sample hack; should be handled by the base repeater, or a mixin for same
        window.addEventListener('keydown', e => {
            if (e.key === 'Tab') {
                const edgeChild = e.shiftKey ? 'firstChild' : 'lastChild';
                const adjElement = e.shiftKey ? 'nextElementSibling' : 'previousElementSibling';
                let edge = this.container[edgeChild];
                while (edge.nodeType !== 1 || edge.style.display === 'none') {
                    edge = edge[adjElement];
                }
                const edgeChildFocused =
            ...
            newChildFn: (item, idx) => {
                ...
                if (recycled) {
                    ...
                    if (type === 'contact') {
                        ...
                        card.style = 'padding: 10px; border-bottom: 1px solid #CCC; width: 100%; box-sizing: border-box;';
                        ...
                        card.addEventListener(
                            'input',
                            e => this._updateChildSize(card['_idx'], e)
                        );
                        text.addEventListener('focus', e => this._scrollToFocused(e));
                        text.addEventListener(
                            'blur',
                            e => this._commitChange(card['_idx'], 'longText', e.target.textContent)
                        );
                        ...
                        header.style = 'color: white; background: #2222DD; padding: 10px; border-bottom: 1px solid #CCC; width: 100%; box-sizing: border-box;';
                        return header;
                    }
                }
            },
    ...
    // and informing layout of a change in child size, to demo/test related use cases
    ...
    _updateChildSize(idx, {currentTarget}) {
        this.layout.updateChildSizes({
            [idx]: {width: currentTarget.offsetWidth, height: currentTarget.offsetHeight}
        });
    }
}